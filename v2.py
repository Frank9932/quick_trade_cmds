...
def calculate_parameters(stopRate):
        ...
        marg = float(buyRate)-float(stopRate)
        quantity = int(float(riskUnit) / marg)
        # outRate = float(plr)*marg + float(buyRate)
        outRate = round(
            (float(plr)*marg + float(buyRate)),len(str(buyRate).split('.')[1])
            )
        return buyRate,stopRate,outRate,quantity

def calculate_riskUnit():
    riskLevel = 0.01
    availeq = extract_from_dict((accountAPI.get_account('USDT')),"availEq")
    riskUnit = riskLevel*float(availeq)
...
def get_RQ():
    ...
    quantity = int(default_riskLevel*)
    quantity_input.delete(0, tk.END)  # Clear the current input
    quantity_input.insert(0, quantity)  # Set quantity in the input box
...
def perform_action(action):
    if confirm_action():
        ...
        elif action == "Buy PLR SLTP":
            info =plr_sltp("buy")
            pass

        elif action == "Sell PLR SLTP":
            info =plr_sltp("sell")
            pass
...
# crypto_label
crypto_label = tk.Label(frame, text="Sym:",width=10, height=1, justify=tk.RIGHT)
crypto_label.grid(row=0, column=0, padx=0, pady=0)
...
crypto_combobox.configure(width=10)
crypto_combobox.set(crypto_list[0])  # Set the default value    
crypto_combobox.grid(row=0, column=1, padx=0, pady=0,rowspan=1)
...
clipboard_button.grid(row=1, column=2, padx=10, pady=10)
...
get_quantity_button = tk.Button(frame, text="Get Qty", command=get_quantity)
get_quantity_button.grid(row=2, column=2, padx=10, pady=10)
...
get_risk_lot_button.grid(row=0, column=7, padx=10, pady=10)
...
sell_radio = tk.Radiobutton(frame, text="SPOT", variable=option_var, value="SPOT", command=handle_option)
buy_radio.grid(row=0, column=2, padx=10, pady=10)
sell_radio.grid(row=0, column=3, padx=10, pady=10)


# Create an instance of the custom StdoutRedirector class
output_text = tk.Text(frame, width=110, height=20, bg="black", fg="white", font=("Courier", 8))
output_text.grid(row=9, column=0, columnspan=8, padx=5, pady=5)
